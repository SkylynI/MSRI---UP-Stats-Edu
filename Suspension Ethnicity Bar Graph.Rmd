...
colnames(EthTot) <- c("Years", "Total", "Race_Ethnicity")

Ethinfo <- aggregate(.~Years+Race_Ethnicity, data=EthTot, sum)
options(scipen = 999)


EthPlot <- ggplot(Ethinfo, aes(Years,Total)) + labs(title= "Yearly Suspension Counts in California K-12 Schools", subtitle = "By Race/Ethnicity", fill="Race/Ethnicity") + geom_bar(aes(fill = Race_Ethnicity),stat="identity") + theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) + scale_y_continuous(limits=c(0,750000), breaks = seq(0,750000,75000), labels = comma)
